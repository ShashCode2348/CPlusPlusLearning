...
class Cat :public Animal {				 //Inheritance: Cat is a subclass of Animal, and gets all its functions and variables
public:							
	void jump() { cout << "Cat jumping" << endl; } //An addition to the Cat class which wasn't in the Animal class
};

class Tiger : public Cat {			//Subclass of Cat. Inherits Cat's functions, which inherited Animal's functions
public:												
...
public:
	...
	void info() { cout << "My name is: " << getName() << endl; }	//Can access private functions, as private variables can be accessed in class documetation
};
...
class Vehicle :public Machine {		//When creating an object of a subclass, the constructors of its superclasses are called as well in order
public:	
